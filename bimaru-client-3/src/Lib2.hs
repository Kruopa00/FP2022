...
renderList (DMap ((x,DMap []):[])) _ string = string ++  (add' x) ++ ":" ++ " {}"
...
renderList (DMap ((x,DList []):[])) _ string = (string ++  (add' x) ++ ": []\n")
...
renderMap (DMap ((x,DList []):[])) _ string =  (((string ++ (add' x) ++ ": []"))++"\n")
...
renderMap (DMap ((x,DMap []):[])) _ string = (string ++  (add' x) ++ ": {}" ++ "\n")
